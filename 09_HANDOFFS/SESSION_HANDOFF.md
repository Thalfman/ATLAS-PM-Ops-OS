...
Author Gemini-first prompts that pair against the 15 W-NN workflow cards. Each prompt cites the same Phase 3 governance bundle by name (data category from `DATA_SENSITIVITY_DECISION_MODEL.md`, tool environment, review intensity from `HUMAN_REVIEW_AND_AUDITABILITY_MODEL.md`, per-domain review pattern) and the matching workflow card by W-ID. The backlog rows A-0018, A-0019, A-0039 through A-0047 enumerate the Phase 5 prompts; the per-W-NN file §15 "Cross-references" section names the paired prompt's A-ID. As an optional housekeeping pass, build A-0038 Process Gap Note as `W-16-process-gap-note.md` and the matching Phase 5 prompt.
...
Posture:
Markdown-first, Gemini-first, platform-agnostic, human-in-the-loop, approved-tool-first. Do not add apps, package dependencies, APIs, databases, deployment files, or code scaffolding. Author Gemini-first prompts under 05_PROMPTS/ that pair against the 15 W-NN workflow cards in 04_WORKFLOWS/. Each prompt cites the Phase 3 governance bundle by name (data category per DATA_SENSITIVITY_DECISION_MODEL.md, tool environment, review intensity per HUMAN_REVIEW_AND_AUDITABILITY_MODEL.md, per-domain review pattern) and names the paired W-NN card by ID. Backlog rows A-0018, A-0019, A-0039..A-0047 enumerate the Phase 5 prompts; each W-NN card's section 15 names the paired prompt's A-ID. Log Phase 5 decisions in 10_DECISION_LOG/DECISION_LOG.md in the same session. Update 09_HANDOFFS/SESSION_HANDOFF.md at the end with the Phase 6 next best prompt. Optional housekeeping: build A-0038 Process Gap Note Workflow as 04_WORKFLOWS/W-16-process-gap-note.md before or after the Phase 5 prompts.
```
